    ...
    def read_temperature(self, index):
        ...
        if self.temperature_controller and self.temp_controllers_connected:
            try:
                ...
                self.clamp_temp_labels[index].config(foreground='orange')
        else:
                ...
                try:
                    if not self.power_supplies[i].is_connected():
                        ...
                        if self.retry_connection(i):
                            ...
                            self.actual_heater_current_vars[i].config(foreground='gray')
                            self.actual_heater_voltage_vars[i].config(foreground='gray')
                            self.operation_mode_var[i].config(foreground='gray')
                            continue
                    ...
                    self.operation_mode_var[i].set("Mode: --")
                    self.actual_heater_current_vars[i].config(foreground='orange')
                    self.actual_heater_voltage_vars[i].config(foreground='orange')
                    self.operation_mode_var[i].config(foreground='orange')
            else:
                self.actual_heater_current_vars[i].set("-- A")
                self.actual_heater_voltage_vars[i].set("-- V")
